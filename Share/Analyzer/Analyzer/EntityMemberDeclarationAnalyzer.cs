...
namespace ET.Analyzer
{
    ...
    public class EntityMemberDeclarationAnalyzer: DiagnosticAnalyzer
    {
        ...
        /// 检查委托成员
        /// </summary>
        private static void AnalyzeDelegateMember(SemanticModelAnalysisContext context, INamedTypeSymbol namedTypeSymbol)
        {
            ...
            void ReportDiagnostic(ISymbol symbol, string delegateName)
            {
                foreach (var syntaxReference in symbol.DeclaringSyntaxReferences)
                {
                    ...
                    Diagnostic diagnostic = Diagnostic.Create(EntityDelegateDeclarationAnalyzerRule.Rule, syntax.GetLocation(), namedTypeSymbol.Name,
                        delegateName);
            ...
            foreach (var member in namedTypeSymbol.GetMembers())
            {
                ...
                if (fieldSymbol.Type is IArrayTypeSymbol arrayTypeSymbol)
                {
                    if (arrayTypeSymbol.ElementType.IsETEntity())
                    {
                        ...
                        Diagnostic diagnostic = Diagnostic.Create(EntityFieldDeclarationInEntityAnalyzerRule.Rule,
                            syntaxReference.GetSyntax().GetLocation(), namedTypeSymbol.Name, fieldSymbol.Name);
                        context.ReportDiagnostic(diagnostic);
                    }
        ...
        private void AnalyzeFloatMemberInLSEntity(SemanticModelAnalysisContext context, INamedTypeSymbol namedTypeSymbol)
        {
            foreach (var member in namedTypeSymbol.GetMembers())
            {
                ...
                if (memberType.SpecialType is SpecialType.System_Single or SpecialType.System_Double)
                {
                    ...
                    Diagnostic diagnostic = Diagnostic.Create(LSEntityFloatMemberAnalyzer.Rule, syntaxReference.GetSyntax().GetLocation(),
                        namedTypeSymbol.Name, member.Name);
                    context.ReportDiagnostic(diagnostic);
                    continue;
                }
        ...
        private void AnalyzeComponentChildAttr(SemanticModelAnalysisContext context, INamedTypeSymbol namedTypeSymbol)
        {
            ...
            if (hasComponentOf && hasChildOf)
            {
                ...
                Diagnostic diagnostic =
                        Diagnostic.Create(EntityComponentChildAnalyzerRule.Rule, syntax?.Identifier.GetLocation(), namedTypeSymbol.Name);
                context.ReportDiagnostic(diagnostic);
            }
        }
