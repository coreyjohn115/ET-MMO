    ...
    public class StaticClassCircularDependencyAnalyzer: DiagnosticAnalyzer
    {
        ...
        private void CompilationStartAnalysis(CompilationStartAnalysisContext context)
        {
            ...
            if (AnalyzerHelper.IsAssemblyNeedAnalyze(context.Compilation.AssemblyName, AnalyzeAssembly.AllHotfix))
            {
                ...
                {
                    this.CircularDependencyAnalyze(analysisContext, dependencyMap, staticClassSet);
                });
            }
        }
        ...
        /// </summary>
        private void StaticClassDependencyAnalyze(SyntaxNodeAnalysisContext context,
        ConcurrentDictionary<string, Dictionary<string, List<InvocationExpressionSyntax>>> dependencyMap,
        ...
        /// 环形依赖分析
        /// </summary>
        private void CircularDependencyAnalyze(CompilationAnalysisContext context,
        ConcurrentDictionary<string, Dictionary<string, List<InvocationExpressionSyntax>>> dependencyMap,
        HashSet<string> staticClassSet)
        {
            // 排除只引用其他静态类的静态类
        ...
        /// 获取没有被任何其他静态类引用的静态类
        /// </summary>
        private string GetClassNotReferencedByOtherStaticClass(
        ConcurrentDictionary<string, Dictionary<string, List<InvocationExpressionSyntax>>> dependencyMap,
        HashSet<string> staticClassSet)
        {
            foreach (string? staticClass in staticClassSet)
        ...
        /// 获取没有引用任何其他静态类的静态类
        /// </summary>
        private string GetClassNotReferenceAnyStaticClass(
        ConcurrentDictionary<string, Dictionary<string, List<InvocationExpressionSyntax>>> dependencyMap,
        HashSet<string> staticClassSet)
        {
            foreach (string? staticClass in staticClassSet)
        ...
        private void FindAllCycles(string currentClass,
        ConcurrentDictionary<string, Dictionary<string, List<InvocationExpressionSyntax>>> dependencyMap,
