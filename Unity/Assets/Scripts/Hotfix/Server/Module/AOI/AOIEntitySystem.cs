...
namespace ET.Server
{
    ...
    public static partial class AOIEntitySystem2
    {
        ...
        {
            self.GetParent<Unit>().GetComponent<NumericComponent>().SetNoEvent(NumericType.AOI, 15000); //视野15米
    ...
    {
        // 获取在自己视野中的对象
        public static Dictionary<long, EntityRef<Unit>> GetSeeUnits(this AOIEntity self)
        {
            return self.SeeUnits;
        }

        public static Dictionary<long, EntityRef<Unit>> GetBeSeePlayers(this AOIEntity self)
        {
            return self.BeSeePlayers;
        }

        public static Dictionary<long, EntityRef<Unit>> GetSeePlayers(this AOIEntity self)
        {
        ...
        public static void EnterSight(this AOIEntity self, AOIEntity enter)
        {
            ...
            if (self.Unit.Type() == UnitType.Player)
            {
                ...
                {
                    self.SeeUnits.Add(enter.Id, enter.Unit);
                    enter.BeSeeUnits.Add(self.Id, self.Unit);
                    self.SeePlayers.Add(enter.Id, enter.Unit);
                    enter.BeSeePlayers.Add(self.Id, self.Unit);
