using System;
using System.Collections.Generic;
using System.IO;

namespace ET
{
    public class CircularBuffer: Stream
    {
        public int ChunkSize => this.chunkSize;

        public int LastIndex { get; set; }

        public int FirstIndex { get; set; }

        private readonly int chunkSize = 8192;
        private byte[] lastBuffer;
        private readonly Queue<byte[]> bufferQueue = new();
        private readonly Queue<byte[]> bufferCache = new();

        public override long Length
        {
            get
            {
                int c = 0;
                if (this.bufferQueue.Count == 0)
                {
                    c = 0;
                }
                else
                {
                    c = (this.bufferQueue.Count - 1) * this.chunkSize + this.LastIndex - this.FirstIndex;
                }

                if (c < 0)
                {
                    Log.Error("CircularBuffer count < 0: {0}, {1}, {2}".Fmt(this.bufferQueue.Count, this.LastIndex, this.FirstIndex));
                }

                return c;
            }
        }

        public byte[] First
        {
            get
            {
                if (this.bufferQueue.Count == 0)
                {
                    this.AddLast();
                }

                return this.bufferQueue.Peek();
            }
        }

        public byte[] Last
        {
            get
            {
                if (this.bufferQueue.Count == 0)
                {
                    this.AddLast();
                }

                return this.lastBuffer;
            }
        }

        public CircularBuffer()
        {
            this.AddLast();
        }

        public void AddLast()
        {
            byte[] buffer = this.bufferCache.Count > 0? this.bufferCache.Dequeue() : new byte[this.chunkSize];
            this.bufferQueue.Enqueue(buffer);
            this.lastBuffer = buffer;
        }

        public void RemoveFirst()
        {
            this.bufferCache.Enqueue(bufferQueue.Dequeue());
        }

        // 从CircularBuffer读到stream
        public void Read(Stream stream, int count)
        {
            if (count > this.Length)
            {
                throw new Exception($"bufferList length < count, {Length} {count}");
            }

            int alreadyCopyCount = 0;
            while (alreadyCopyCount < count)
            {
                int n = count - alreadyCopyCount;
                if (this.FirstIndex + n < this.chunkSize)
                {
                    stream.Write(this.First, this.FirstIndex, n);
                    this.FirstIndex += n;
                    alreadyCopyCount += n;
                }
                else
                {
                    int length = this.chunkSize - this.FirstIndex;
                    stream.Write(this.First, this.FirstIndex, length);
                    alreadyCopyCount += length;
                    this.FirstIndex = 0;
                    this.RemoveFirst();
                }
            }
        }

        // 从stream写入CircularBuffer
        public void Write(Stream stream)
        {
            int count = (int)(stream.Length - stream.Position);
            int alreadyCopyCount = 0;
            while (alreadyCopyCount < count)
            {
                if (this.LastIndex == this.chunkSize)
                {
                    this.AddLast();
                    this.LastIndex = 0;
                }

                int n = count - alreadyCopyCount;
                if (this.LastIndex + n < this.chunkSize)
                {
                    int read = stream.Read(this.lastBuffer, this.LastIndex, n);
                    this.LastIndex += read;
                    alreadyCopyCount += read;
                }
                else
                {
                    int read = stream.Read(this.lastBuffer, this.LastIndex, this.chunkSize - this.LastIndex);
                    alreadyCopyCount += read;
                    this.LastIndex = this.chunkSize;
                }
            }
        }

        // 把CircularBuffer中数据写入buffer
        public override int Read(byte[] buffer, int offset, int count)
        {
            if (buffer.Length < offset + count)
            {
                throw new Exception($"bufferList length < count, buffer length: {buffer.Length} {offset} {count}");
            }

            long length = this.Length;
            if (length < count)
            {
                count = (int)length;
            }

            int alreadyCopyCount = 0;
            while (alreadyCopyCount < count)
            {
                int n = count - alreadyCopyCount;
                if (ChunkSize - this.FirstIndex > n)
                {
                    Array.Copy(this.First, this.FirstIndex, buffer, alreadyCopyCount + offset, n);
                    this.FirstIndex += n;
                    alreadyCopyCount += n;
                }
                else
                {
                    Array.Copy(this.First, this.FirstIndex, buffer, alreadyCopyCount + offset, ChunkSize - this.FirstIndex);
                    alreadyCopyCount += ChunkSize - this.FirstIndex;
                    this.FirstIndex = 0;
                    this.RemoveFirst();
                }
            }

            return count;
        }

        // 把buffer写入CircularBuffer中
        public override void Write(byte[] buffer, int offset, int count)
        {
            int alreadyCopyCount = 0;
            while (alreadyCopyCount < count)
            {
                if (this.LastIndex == this.chunkSize)
                {
                    this.AddLast();
                    this.LastIndex = 0;
                }

                int n = count - alreadyCopyCount;
                if (this.chunkSize - this.LastIndex > n)
                {
                    Array.Copy(buffer, alreadyCopyCount + offset, this.lastBuffer, this.LastIndex, n);
                    this.LastIndex += count - alreadyCopyCount;
                    alreadyCopyCount += n;
                }
                else
                {
                    Array.Copy(buffer, alreadyCopyCount + offset, this.lastBuffer, this.LastIndex, this.chunkSize - this.LastIndex);
                    alreadyCopyCount += this.chunkSize - this.LastIndex;
                    this.LastIndex = this.chunkSize;
                }
            }
        }

        public override void Flush()
        {
            throw new NotImplementedException();
        }

        public override long Seek(long offset, SeekOrigin origin)
        {
            throw new NotImplementedException();
        }

        public override void SetLength(long value)
        {
            throw new NotImplementedException();
        }

        public override bool CanRead
        {
            get
            {
                return true;
            }
        }

        public override bool CanSeek
        {
            get
            {
                return false;
            }
        }

        public override bool CanWrite
        {
            get
            {
                return true;
            }
        }

        public override long Position { get; set; }
    }
}