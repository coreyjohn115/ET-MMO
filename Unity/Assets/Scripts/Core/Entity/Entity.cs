...
namespace ET
{
    ...
    public abstract partial class Entity: DisposeObject, IPool
    {
        ...
        public Entity GetChildByName(string typeName)
        {
            if (this.children == null)
            {
                return null;
        ...
        /// <returns></returns>
        public Entity AddChildByType<A>(A a, Type type, bool isFromPool = false)
        {
            Entity entity = Create(type, isFromPool);
            entity.Id = IdGenerater.Instance.GenerateId();
            entity.Parent = this;

            EntitySystemSingleton.Instance.Awake(entity, a);
            return entity;
        }
