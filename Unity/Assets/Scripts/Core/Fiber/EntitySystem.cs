...
namespace ET
{
    public class EntitySystem
    {
        ...
        private readonly Dictionary<long, Entity> entityDict = new(1000);
        ...
        {
            return this.entityDict.Get(instanceId);
        }
        ...
        {
            var queue = this.queues[InstanceQueueIndex.LateUpdate];
            int count = queue.Count;
            while (count-- > 0)
            {
                Entity component = queue.Dequeue();
        ...
        public void FixedUpdate()
        {
            ...
            while (count-- > 0)
            {
                ...
                queue.Enqueue(component);
                foreach (IFixedUpdateSystem iLateUpdateSystem in iLateUpdateSystems)
                {
                    try
                    {
                        iLateUpdateSystem.Run(component);
        ...
        public void Load()
        {
            var queue = this.queues[InstanceQueueIndex.Load];
            int count = queue.Count;
            while (count-- > 0)
            {
                Entity component = queue.Dequeue();
