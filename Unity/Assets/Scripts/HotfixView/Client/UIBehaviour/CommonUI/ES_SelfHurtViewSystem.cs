...
namespace ET.Client
{
    [EntitySystemOf(typeof (ES_SelfHurt))]
    [FriendOf(typeof (ES_SelfHurt))]
    public static partial class ES_SelfHurtSystem
    {
        [EntitySystem]
        private static void Awake(this ES_SelfHurt self, Transform transform)
        {
        ...
        [EntitySystem]
        private static void Destroy(this ES_SelfHurt self)
        {
        ...
        public static void Initliaze(this ES_SelfHurt self, long caster, HurtProto proto)
        {
            ...
            Tweener moveXL = self.E_TextText.rectTransform.DOAnchoredPositionX(-100f, 0.3f, 0f).SetEase(Ease.Linear);
            Tweener moveYL = self.E_TextText.rectTransform.DOAnchoredPositionY(100f, 0.3f, 0f).SetEase(Ease.Linear);
            Tweener scaleL = self.E_TextText.rectTransform.DOScale(1f, 0.3f, 0.1f).SetEase(Ease.OutQuad);
            Tweener moveXL2 = self.E_TextText.rectTransform.DOAnchoredPositionX(-150f, 0.6f, -100f).SetEase(Ease.InCubic);
            Tweener moveYL2 = self.E_TextText.rectTransform.DOAnchoredPositionY(50f, 0.6f, 100f).SetEase(Ease.InCubic);
            Tweener alphaL = self.collector.Get<CanvasGroup>("CanvasGroup").DOFade(0f, 0.6f).SetEase(Ease.InQuad);
        ...
        private static void OnPlay(this ES_SelfHurt self, Tweener tweener)
        {
        ...
        private static void OnUpdate(this ES_SelfHurt self, Tweener tweener)
        {
            Vector2 pos = Vector2.one;
            UIHelper.WorldToUI(self.startWorldPos, ref pos);
            (self.uiTransform as RectTransform).anchoredPosition = pos;
        ...
        private static void OnComplete(this ES_SelfHurt self, Tweener tweener)
        {
            tweener?.Reset();
            self.Dispose();
        }

        public static void Play(this ES_SelfHurt self)
        {
