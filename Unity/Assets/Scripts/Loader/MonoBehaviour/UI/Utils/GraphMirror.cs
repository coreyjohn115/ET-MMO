...
namespace ET
{
    ...
    /// 镜像图片
    /// </summary>
    [AddComponentMenu("UI/Effects/Gragh Mirror", 20)]
    [RequireComponent(typeof(Graphic))]
    public class GraphMirror : BaseMeshEffect
    {
        #region Public Properties
        /// <summary>
        /// 镜像类型
        /// </summary>
        public GraphMirrorType MirrorType
        {
            get => mirrorType;
            set
            {
        ...
        {
            if (graphic != null && graphic is Image image)
            {
                ...
                if (overrideSprite != null)
                {
                    ...
                    switch (mirrorType)
                    {
                        case GraphMirrorType.Horizontal:
                            Trans.sizeDelta = new Vector2(w * 2, h);
                            break;
                        case GraphMirrorType.Vertical:
                            Trans.sizeDelta = new Vector2(w, h * 2);
                            break;
                        case GraphMirrorType.Quarter:
                            Trans.sizeDelta = new Vector2(w * 2, h * 2);
                            break;
                    }
        ...
        public override void ModifyMesh(VertexHelper vh)
        {
            ...
            var output = (List<UIVertex>)ObjectPool.Instance.Fetch(typeof(List<UIVertex>));
            ...
            ObjectPool.Instance.Recycle(output);
        }
        #endregion
        ...
        protected void DrawTiled(List<UIVertex> verts, int count)
        {
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                if (mirrorType == GraphMirrorType.Horizontal || mirrorType == GraphMirrorType.Quarter)
                {
                    //判断三个点的水平位置是否在偶数矩形内，如果是，则把UV坐标水平翻转
                    if (Mathf.FloorToInt((centerX - rect.xMin) / w) % 2 == 1)
                    {
                        v1.uv0 = GetOverturnUV(v1.uv0, inner.x, inner.z);
                        v2.uv0 = GetOverturnUV(v2.uv0, inner.x, inner.z);
                        v3.uv0 = GetOverturnUV(v3.uv0, inner.x, inner.z);
                    }
                }

                if (mirrorType == GraphMirrorType.Vertical || mirrorType == GraphMirrorType.Quarter)
                {
        ...
        private RectTransform Trans =>
            this.rectTransform? this.rectTransform : (this.rectTransform = this.GetComponent<RectTransform>());
        #endregion

        #region Internal Fields
        [SerializeField]
        private GraphMirrorType mirrorType = GraphMirrorType.Horizontal;
