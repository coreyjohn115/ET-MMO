...
namespace ET.Client
{
    public class UnityObjectViewer: DebugWindowBase
    {
        ...
        protected override void OnDrawWindow(int id)
        {
            ...
            var rootGameObjects = this._selectedScene == 0? this.getDontDestroyOnLoadGameObjects()
                ...
                if (this._showPublic)
                {
                    ...
                    foreach (FieldInfo fieldInfo in fields)
                    {
                        ...
                        if (attr != null)
                            continue;
                        GUILayout.TextField(
                            $"<color=white>{fieldInfo.Name}</color> <color=yellow>{fieldInfo.FieldType}</color> <color=#B7FF4D>{fieldInfo.GetValue(com)}</color>",
                            skin);
                    }
                    ...
                    if (this._showProperty)
                    {
                        ...
                        foreach (var propertyInfo in properties)
                        {
                            ...
                            if (attr != null)
                                continue;
                            GUILayout.TextField(
                                $"<color=white>{propertyInfo.Name}</color> <color=yellow>{propertyInfo.PropertyType}</color> <color=#B7FF4D>{propertyInfo.GetValue(com)}</color>",
                                skin);
                        }
                    }
                }
